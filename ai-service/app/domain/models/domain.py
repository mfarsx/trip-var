...
class TravelPreferences(BaseModel):
    ...
    destination: str = Field(
        ..., min_length=2, max_length=100, description="Destination city or country"
    )
    ...
    travel_style: Optional[TravelStyle] = Field(
        None, description="Preferred travel style"
    )
    num_travelers: int = Field(
        default=1, ge=1, le=20, description="Number of travelers (max 20)"
    )
...
class DayPlan(BaseModel):
    ...
    estimated_cost: Optional[str] = Field(
        None, max_length=100, description="Estimated cost for the day's activities"
    )
    notes: Optional[str] = Field(
        None, max_length=500, description="Additional notes or tips for the day"
    )
...
class TravelPlan(BaseModel):
    ...
    overview: str = Field(
        ..., min_length=50, max_length=1000, description="Overview of the entire trip"
    )
    highlights: List[str] = Field(
        ..., min_items=1, max_items=10, description="Key highlights of the trip"
    )
    daily_plans: List[DayPlan] = Field(
        ..., min_items=1, max_items=30, description="Detailed day-by-day plans"
    )
    total_estimated_cost: Optional[str] = Field(
        None, max_length=100, description="Total estimated cost for the trip"
    )
    packing_suggestions: List[str] = Field(
        default=[], max_items=20, description="Suggested items to pack"
    )
    travel_tips: List[str] = Field(
        default=[], max_items=10, description="General travel tips for the destination"
    )
    ...
    def validate_daily_plans(cls, v: List[DayPlan]) -> List[DayPlan]:
        ...
        for i, plan in enumerate(v, 1):
            if plan.day != i:
                raise ValueError(
                    f"Day plans must be in sequence. Expected day {i}, got {plan.day}"
                )
        return v
