...
class Logger {
  ...
  debug(message, data) {
    if (this.isDevelopment) {
      const { messageString, color } = this._formatMessage(LOG_LEVELS.DEBUG, message, data);
      console.debug(`%c${messageString}`, `color: ${color}`);
    }
  }

  info(message, data) {
    const { messageString, color } = this._formatMessage(LOG_LEVELS.INFO, message, data);
    console.info(`%c${messageString}`, `color: ${color}`);
  }

  warn(message, data) {
    const { messageString, color } = this._formatMessage(LOG_LEVELS.WARN, message, data);
  ...
  error(message, error) {
    ...
    const { messageString, color } = this._formatMessage(LOG_LEVELS.ERROR, message, data);
