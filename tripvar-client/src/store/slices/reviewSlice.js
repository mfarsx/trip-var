  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(createReview.fulfilled, (state, action) => {
        ...
        const newAverage = ((currentStats.averageRating * currentStats.totalReviews) + newReview.rating) / totalReviews;
        ...
        state.ratingStats = {
          ...
          distribution: {
            ...
            [newReview.rating]: (currentStats.distribution[newReview.rating] || 0) + 1
          }
        };
      })
      ...
      .addCase(updateReview.fulfilled, (state, action) => {
        ...
        const index = state.destinationReviews.findIndex(review => review._id === updatedReview._id);
        if (index !== -1) {
      ...
      .addCase(deleteReview.fulfilled, (state, action) => {
        ...
        const deletedReview = state.destinationReviews.find(review => review._id === deletedReviewId);
        
        // Remove from both lists
        state.destinationReviews = state.destinationReviews.filter(review => review._id !== deletedReviewId);
        state.userReviews = state.userReviews.filter(review => review._id !== deletedReviewId);
        ...
        if (deletedReview) {
          ...
          if (totalReviews === 0) {
            ...
            // Update distribution
            const newDistribution = { ...currentStats.distribution };
            newDistribution[deletedReview.rating] = Math.max(0, (newDistribution[deletedReview.rating] || 0) - 1);
